    ...
    public class UserLoginTask extends AsyncTask<Void, Integer, Integer> {
        ...
        protected Integer doInBackground(Void... params) {
            ...
            try {
                //登陆
                if (isLogin) {
                    responseCode = HttpUtil.loginVerify(HttpUtil.URL_LOGIN, mUsername, mPassword);
                } else {
                    responseCode = HttpUtil.registerAccount(HttpUtil.URL_REGISTER, mUsername, mPassword);
                }
            } catch (IOException e) {
                e.printStackTrace();
                return 0;
        ...
        protected void onPostExecute(Integer result) {
            ...
            switch (result) {
                case 200:
                    succeedExecute();
                    Toast.makeText(LoginActivity.this, "欢迎：" + mUsername, Toast.LENGTH_LONG).show();
                    finish();
                    break;
                case 201:
                ...
                case 401:
                    ...
                    break;
                case 402:
                    mPasswordView.setError("密码有误");
                    mPasswordView.requestFocus();
                    break;
                case 403:
                    mUsernameView.setError("用户名有误");
                    mUsernameView.requestFocus();
                    break;
                case 405:
                    mUsernameView.setError("这个用户名已经被注册");
                    mUsernameView.requestFocus();
                    break;
            }
        }
